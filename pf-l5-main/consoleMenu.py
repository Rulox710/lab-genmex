...
from util import Color, SpecialCharacter
...
class InteractiveConsole:
  ...
  def ask_menu(options: List[str], start: int, end: int, **kwargs) -> int:
    """
    ...
    Args:
      ...
      end(int): Indice donde se finaliza de recorrer el arreglo.
    ...
    while(loop):
      ...
      if option <= (end - start + 1) and option >= 1: loop = False
      if error:
      ...
      if loop and not(error):
        ...
        Printer.print_error(SpecialCharacter.PREV_LINE.value.format(0),
                           InteractiveConsole.GENERIC_ANSWERS['RANGE'], end='\n')
    spaces = 3 if error else 2
    print(SpecialCharacter.PREV_LINE.value.format(spaces),
          SpecialCharacter.CLEAR_FROM_CURSOR.value,sep='',end='')
    Printer.print_ok(InteractiveConsole.GENERIC_ANSWERS['ACCEPT'].format(options[option + start -1]))
    return option
